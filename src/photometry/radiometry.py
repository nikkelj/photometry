...
def facet_brightness(
    model: FacetModel, u_sun_body: np.ndarray, u_obs_body: np.ndarray
) -> np.ndarray:
    ...
    # reflection of the sun direction about each facet normal: (F,K,3)
...
def apparent_magnitude(
    ...
    b_total = facet_brightness(model, u_sun_body, u_obs_body).sum(axis=0)  # m^2/sr-ish
